...
const TvlMain: FC<onClickProps> = ({ Connect, account }) => {
  ...
  useEffect(() => {
    const poolData = async () => {
      setIsLoading(true);
      let res = `http://54.67.110.239:8080/dashboard/yield?sort=tvl&cursor=999999999`;
  ...
  const poolSaveData = async () => {
    setIsLoading(true);
    let res = `http://54.67.110.239:8080/dashboard/yield?sort=tvl&cursor=${nextCursorData}`;
    let response = await fetch(res);
    let tvlData = await response.json();
